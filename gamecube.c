...
static char gc_analog_lr_disable = 0;
...
static char gamecubeUpdate(void)
{
	...
	count = gcn64_transaction(tmpdata, 3,port_id);
 ...
 */
	...
	btns2 = gcn64_protocol_getByte(8,port_id);
	x = gcn64_protocol_getByte(16,port_id);
	y = gcn64_protocol_getByte(24,port_id);
	cx = gcn64_protocol_getByte(32,port_id);
	cy = gcn64_protocol_getByte(40,port_id);
	ltrig = gcn64_protocol_getByte(48,port_id);
	rtrig = gcn64_protocol_getByte(56,port_id);
...
static char gamecubeBuildReport(unsigned char *reportBuffer, int id)
{
...
static void gamecubeSetID(unsigned char id)
{
	if(id > PORT_2)
		id = PORT_2;
